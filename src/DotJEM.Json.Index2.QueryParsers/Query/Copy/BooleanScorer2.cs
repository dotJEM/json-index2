...
using CopyOfBooleanWeight = DotJEM.Json.Index2.QueryParsers.Query.Copy.CopyOfBooleanQuery.CopyOfBooleanWeight;

namespace DotJEM.Json.Index2.QueryParsers.Query.Copy;

internal class CopyOfBooleanScorer2 : Scorer
{
    ...
    private class Coordinator
    {
        ...
        internal Coordinator(CopyOfBooleanScorer2 outerInstance, int maxCoord, bool disableCoord)
        {
            coordFactors = new float[outerInstance.optionalScorers.Count + outerInstance.requiredScorers.Count + 1];
            for (int i = 0; i < coordFactors.Length; i++)
            {
                coordFactors[i] = disableCoord ? 1.0f : ((CopyOfBooleanWeight)outerInstance.m_weight).Coord(i, maxCoord);
            }
        }
    ...
    public CopyOfBooleanScorer2(CopyOfBooleanWeight weight, bool disableCoord, int minNrShouldMatch, IList<Scorer> required, IList<Scorer> prohibited, IList<Scorer> optional, int maxCoord)
        : base(weight)
    {
    ...
    {
        private readonly CopyOfBooleanScorer2 outerInstance;
        ...
        internal SingleMatchScorer(CopyOfBooleanScorer2 outerInstance, Scorer scorer)
            : base(scorer.Weight)
        {
    ...
    private sealed class MinShouldMatchSumScorerAnonymousClass : CopyOfMinShouldMatchSumScorer
    {
        private readonly CopyOfBooleanScorer2 outerInstance;

        public MinShouldMatchSumScorerAnonymousClass(CopyOfBooleanScorer2 outerInstance, Weight weight, IList<Scorer> scorers, int minNrShouldMatch)
    ...
    private sealed class DisjunctionSumScorerAnonymousClass : CopyOfDisjunctionSumScorer
    {
        private readonly CopyOfBooleanScorer2 outerInstance;

        public DisjunctionSumScorerAnonymousClass(CopyOfBooleanScorer2 outerInstance, Weight weight, Scorer[] subScorers, float[] coord)
    ...
    private sealed class ConjunctionScorerAnonymousClass : CopyOfConjunctionScorer
    {
        private readonly CopyOfBooleanScorer2 outerInstance;

        private readonly int requiredNrMatchers;

        public ConjunctionScorerAnonymousClass(CopyOfBooleanScorer2 outerInstance, Weight weight, Scorer[] scorers, int requiredNrMatchers)
            : base(weight, scorers)
        {
            this.outerInstance = outerInstance;
    ...
    {
        return new CopyOfConjunctionScorer(m_weight, new Scorer[] { req1, req2 });
    ...
    private Scorer MakeCountingSumScorerSomeReq(/* bool disableCoord // LUCENENET: Not Referenced */) // At least one required scorer.
    {
        ...
        else
        {
            ...
            {
                return new CopyOfReqOptSumScorer(AddProhibitedScorers(requiredCountingSumScorer), optionalScorers.Count == 1 ? new SingleMatchScorer(this, optionalScorers[0])
    ...
    private Scorer AddProhibitedScorers(Scorer requiredCountingSumScorer)
    {
        return (prohibitedScorers.Count == 0) 
            ...
            : new CopyOfReqExclScorer(requiredCountingSumScorer, ((prohibitedScorers.Count == 1) 
                ? prohibitedScorers[0] 
                : new CopyOfMinShouldMatchSumScorer(m_weight, prohibitedScorers))); // no prohibited
    }
