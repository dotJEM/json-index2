...
namespace DotJEM.Json.Index2.QueryParsers.Query.Copy
{
    ...
    internal class CopyOfConjunctionScorer : Scorer
    {
        ...
        internal CopyOfConjunctionScorer(Weight weight, Scorer[] scorers)
            : this(weight, scorers, 1f)
        {
        }

        internal CopyOfConjunctionScorer(Weight weight, Scorer[] scorers, float coord)
            : base(weight)
        {
            this.coord = coord;
            ...
            // lead the matching.
            ArrayUtil.TimSort(m_docsAndFreqs, System.Collections.Generic.Comparer<DocsAndFreqs>.Create((o1, o2) =>
            {
                if (o1.Cost < o2.Cost)
                {
                    return -1;
